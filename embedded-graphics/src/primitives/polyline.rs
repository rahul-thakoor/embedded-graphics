...
use crate::{
    geometry::Point,
    primitives::line::{Line, LineIterator},
};
...
impl<'a> Polyline<'a> {
    ...
    /// Get the total number of points in this polyline
...
impl<'a> Iterator for PolylineIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(p) = self.segment_iter.next() {
            ...
            if self.end_idx == self.line.len() {
                return None;
            }

            self.segment_iter = LineIterator::new(&Line::new(
                self.line.points[self.start_idx],
                self.line.points[self.end_idx],
            ));
...
impl<'a> IntoIterator for Polyline<'a> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        ...
        Self::IntoIter {
            ...
            end_idx,
            line: self,
            segment_iter: LineIterator::new(&Line::new(
